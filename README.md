...
🔍 3. 주요 SQL 활용 예시
데이터 관리 (CRUD)
 데이터 입력: INSERT INTO jdbcBoard VALUES(jpaBoard_seq.nextval, '제목', '내용', '작성자', sysdate);
 전체 목록 조회: SELECT * FROM jpaBoard ORDER BY no DESC;
 게시글 수정: UPDATE jpaBoard SET title='수정제목', content='수정내용' WHERE no=5;
 게시글 삭제: DELETE FROM jpaBoard WHERE no=8;
 검색 기능SQL-- 제목에 'b'가 포함된 게시글 검색
   SELECT * FROM jpaBoard WHERE no > 0 AND title LIKE '%b%';

-- 작성자에 'c'가 포함된 게시글 검색
SELECT * FROM jpaBoard WHERE no > 0 AND writer LIKE '%c%';
⚠️ 주의사항
실습 중 데이터가 잘못되었다면 ROLLBACK;
명령어로 되돌릴 수 있습니다.
데이터 확정을 위해서는 COMMIT;
명령어를 잊지 마세요.
